...
int main(int argc, char *argv[])
{
	...
	bool filltex=true;		// if true then fill invald nodes
	int	method=BRUTE;			// default method using threads
	...
	bool gpuMode=false;         // if use of the GPU is needed
	...
	try
	{
		...
		TCLAP::CmdLine cmd("Acme-Presto Cortical Thickness Normal Distance Processor\n "
		    "Using an outer gray matter mesh and an inner white matter mesh "
		    "RicCorticalThicknessByNormal calculates the average cortical "
		    "thickness as well as creating a texture and mesh of the thicknesses.\n "
		    "GPU mode hardware requirements:\n"
		    "NVIDIA Cuda Capable Graphics Card with minimum architecture of 2.0X\n"
		    "Graphics Cards tested as of 07/09/2016:\n"
		    "NVIDIA Tesla K20 arch 3.0X \n"
		    "This version of RicCorticalThicknessByNormal has a mode where it can calculate the thickness texture in parallel using a CUDA capable GPU.  "
		    "By using this option computation time is greatly decreased.  "
		    "However, it comes with requirements that the user's computer has at least a 2.0X nvidia cuda GPU.  "
		    "If your computer has such a card and the output is incorrect try closing any processes and applications that use the GPU.",\
			 ' ', "0.5");
		...
		TCLAP::SwitchArg ft( "","ft", "Fill invalid nodes in thickness texture", false);
		cmd.add(ft);
		...
		TCLAP::SwitchArg gpu( "","gpu", "Use CUDA capable GPU for calculation require 2.0X nvidia cuda GPU", false);
		cmd.add(gpu);
...
///////////////////////////// Step 1 - Read Files ////////////////////////////
	...
	RicMesh wm_mesh((char*)gm_name.c_str());
...
/////////////////////// Step 3 - Calculate Thickness  ////////////////////////
	...
	if ( surfmap == MAP_WHITE || surfmap == MAP_BOTH) // map thickness on white
	{
		...
		else // the default
		{
			...
			if(gpuMode)
				FindNormalDistThreads_GPU(&wm_mesh, &gm_mesh, &cv_mesh_wm, thick_map_wm, 2, maxd, nflip, mind, maxd);
			else
	...
	if ( surfmap == MAP_GRAY || surfmap == MAP_BOTH) // map on gray
	{
		...
		else // the default
		{
			...
			if(gpuMode)
				FindNormalDistThreads_GPU(&gm_mesh, &wm_mesh, &cv_mesh_gm, thick_map_gm, 2, maxd, -1*nflip, mind, maxd);
			else
