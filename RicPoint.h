...
public:
	...
	// Ctors
	CUDA_PREFIX Point( float InX, float InY, float InZ ) : x( InX ), y( InY ), z( InZ )
	{
	}
	CUDA_PREFIX Point( ) : x(0), y(0), z(0)
	{
	}

	// Operator Overloads
	CUDA_PREFIX inline bool operator== (const Point& V2) const
	{
		return (x == V2.x && y == V2.y && z == V2.z);
	}

	CUDA_PREFIX inline Point operator+ (const Point& V2) const
	{
	...
	CUDA_PREFIX inline Point operator- ( ) const
	{
		return Point(-x, -y, -z);
	}

	CUDA_PREFIX inline Point operator/ (float S ) const
	{
		float fInv = 1.0f / S;
	...
	CUDA_PREFIX inline void operator+= ( const Point& V2 )
	{
		x += V2.x;
		y += V2.y;
		z += V2.z;
	...
	CUDA_PREFIX inline float operator[] ( int i )
	{
		if ( i == 0 ) return x;
	...
	// Functions
	CUDA_PREFIX inline float Dot( const Point &V1 ) const
	{
		return V1.x*x + V1.y*y + V1.z*z;
	}

	CUDA_PREFIX inline Point CrossProduct( const Point &V2 ) const
	{
		return Point(
			y * V2.z  -  z * V2.y,
			z * V2.x  -  x * V2.z,
	...
	CUDA_PREFIX Point RotByMatrix( const float m[16] ) const
	{
		return Point( 
			x*m[0] + y*m[4] + z*m[8],
			x*m[1] + y*m[5] + z*m[9],
	...
	CUDA_PREFIX float Magnitude( ) const
	{
		return sqrtf( x*x + y*y + z*z );
	}

	CUDA_PREFIX float Distance( const Point &V1 ) const
	{
