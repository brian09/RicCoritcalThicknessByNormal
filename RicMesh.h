...
class vertex
{
	...
	CUDA_PREFIX vertex(float x, float y, float z)
	{
	...
	CUDA_PREFIX inline void assign(float x, float y, float z)
	{
		pnt.x=x;pnt.y=y;pnt.z=z;
	}

	/// assign point to a vertex
	CUDA_PREFIX inline void assign(Point p)
	{
		pnt = p;
	}

	/// set vertex point from passed vertex
	CUDA_PREFIX inline void set(vertex fu)
	{
		pnt = fu.pnt;
	}

	/// return the dot product of vertex with passed vertex
	CUDA_PREFIX inline float dot(vertex fu)
	{
	...
	CUDA_PREFIX inline void add(vertex v)
	{
		pnt.x+=v.pnt.x;
		pnt.y+=v.pnt.y;
		pnt.z+=v.pnt.z;
	...
	CUDA_PREFIX inline void minus(vertex v)
	{
		pnt.x-=v.pnt.x;
		pnt.y-=v.pnt.y;
		pnt.z-=v.pnt.z;
	...
	/// subtract passed vertex from vertex
	CUDA_PREFIX inline void subtract(vertex v)
	{
		pnt.x-=v.pnt.x;
		pnt.y-=v.pnt.y;
		pnt.z-=v.pnt.z;
	...
	CUDA_PREFIX inline void divide(float val)
	{
		if ( val == 0 ) return;
		pnt.x/=val;
		pnt.y/=val;
	...
	CUDA_PREFIX inline void multiply(float val)
	{
		pnt.x*=val;
		pnt.y*=val;
		pnt.z*=val;
	...
	CUDA_PREFIX inline void average(vertex fu)
	{
	...
	/// average vertex with passed vertex
	CUDA_PREFIX inline vertex average2(vertex fu)
	{
		vertex v;
	...
	/// normalize vertex
	CUDA_PREFIX inline void normalize()
	{
...
a couple of general purpose integer labes for the triangle.
*/
class triangle
{
	public:
	...
	CUDA_PREFIX triangle(int v1, int v2, int v3)
	{
	...
	CUDA_PREFIX inline void assign(int v1, int v2, int v3)
	{
		vidx[0]=v1; vidx[1]=v2; vidx[2]=v3;
		labeled=0;
		id_labeled=0;
	...
	CUDA_PREFIX inline int check_if_belong(int id, int side, int norm_id)
	{
		for (int i=0;i<3;i++)
		{
			if (vidx[i]==id)
...
class RicMesh
{
	...
	//Converts gifti file to as mesh file
	RicMesh* convertToMesh(char *fname);
